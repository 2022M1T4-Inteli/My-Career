...
# Delcarando a variavel vetor
	if GameManager.horizontalSpeed > 0 and GameManager.verticalSpeed > 0:
		if Input.is_action_pressed("ui_left") and Input.is_action_pressed("ui_up"):
			velocity.x -= GameManager.horizontalSpeed/2
			velocity.y -= 0
		...
		if Input.is_action_pressed("ui_up"):
			velocity.y -= GameManager.verticalSpeed
			stateMachine.travel("Walk")
...
#e a animação da sprite é setada com o estado de movimento.
			...
			velocity.x += GameManager.horizontalSpeed
			stateMachine.travel("Walk")
