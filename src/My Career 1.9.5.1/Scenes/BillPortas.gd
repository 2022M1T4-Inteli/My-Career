...
func _dialog():
	Dialogic.set_variable("esxppoints", 0)
	Dialogic.set_variable("esqzpoints", 0)
...
func _input(event):
	if get_node_or_null('DialogNode') == null: #Verifica se outro diálogo ja esta em cena
		if event.is_action_pressed("ui_E") and activate: #Faz com que a interação ocorra somente se a tecla designada (no caso, a tecla E) for acionada
			...
			var dialog = Dialogic.start(str("BillPortas-",GameManager.bpdialogcount)) #Inicia o diálogo da timeline-1, que foi pré-definida na ferramente "Dialogic"
...
func dialog_listener(string):
	match string:
		...
		"godend-1":
			GameManager.goddialogcount -= 1
		"bpend3":
			GameManager.bpdialogcount += 3
		"bpend-2":
			GameManager.bpdialogcount -= 2
