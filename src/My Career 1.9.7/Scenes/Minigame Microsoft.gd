...
func _ready():
	...
	if GameManager.iniciominigamems == true: #Verifica se outro diálogo ja esta em cena
		...
		var dialog = Dialogic.start(str("RecrutadoraMicrosoft-",GameManager.radialogcount)) #Inicia o diálogo da timeline-1, que foi pré-definida na ferramente "Dialogic"
