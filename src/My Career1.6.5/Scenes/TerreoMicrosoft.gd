...
func _input(event):
	if get_node_or_null('DialogNode') == null: #Verifica se outro diálogo ja esta em cena
		if event.is_action_pressed("ui_E") and activate: #Faz com que a interação ocorra somente se a tecla designada (no caso, a tecla E) for acionada
			...
			var dialog = Dialogic.start(str("RecepcionistaM-",GameManager.msdialogcount)) #Inicia o diálogo da timeline-1, que foi pré-definida na ferramente "Dialogic"
...
func dialog_listener(string):
	match string:
		...
		"msend1":
			#colocar para a area 2d do elevador aparecer
			var x
		"emend0":
